    ...
    <div class="card">
        <ul class="nav nav-tabs row m-0 common-tab">
            <div class="col-sm-3 p-0">
                <li class="nav-item">
                    <a href="{{ route('admin.orders.index') }}?t=pending"
                       class="nav-link @if(Request::get('t')=='pending') active  @endif">@lang('order.dispatcher.pending_title')</a>
                </li>
            </div>
            <div class="col-sm-3 p-0">
                <li class="nav-item">
                    <a href="{{ route('admin.orders.index') }}?t=accepted"
                       class="nav-link @if(Request::get('t')=='accepted') active  @endif">@lang('order.dispatcher.accept_title')</a>
                </li>
            </div>
            <div class="col-sm-3 p-0">
                <li class="nav-item">
                    <a href="{{ route('admin.orders.index') }}?t=ongoing"
                       class="nav-link @if(Request::get('t')=='ongoing') active  @endif">@lang('order.dispatcher.ongoing_title')</a>
                </li>
            </div>
            <div class="col-sm-3 p-0">
                <li class="nav-item">
                    <a href="{{ route('admin.orders.index') }}?t=cancelled"
                       class="nav-link @if(Request::get('t')=='cancelled') active  @endif">@lang('order.dispatcher.cancel_title')</a>
                </li>
            </div>
        </ul>
                            ...
                            <div class="card card-inverse pending-block row m-0 @if($order_now == 1)  bg-primary @elseif($Order->status == 'RECEIVED') bg-danger  @else bg-success @endif">
                                            ...
                                            @if($Order->status == 'RECEIVED')
                                                @if($order_now == 0)
                                                    @if(Setting::get('manual_assign')==1)
                                                        <a href="{{ route('admin.orders.show', $Order->id) }}?t=pending&p=auto"
                                                           class="btn btn-danger btn-darken-3">@lang('order.dispatcher.assign')</a>
                                                    @else
                                                        <a href="{{ route('admin.orders.show', $Order->id) }}?t=pending"
                                                           class="btn btn-danger btn-darken-3">@lang('order.dispatcher.assign')</a>
                                                    @endif
                                                @else
                                ...
                                <div class="delete-block">
                                    ...
                                    <form action="{{ route('admin.orders.update', $Order->id) }}"
                                          id="assign-{{ $Order->id }}" class="form-horizontal" method="POST">
                                        {{ csrf_field() }}
                                        {{ method_field("PATCH") }}
                                ...
                                <div class="card-block">
                                    ...
                                    <div class="col-sm-9">
                                        ...
                                        <div class="card-btm row m-0">
                                            ...
                                            @if($Order->status=='PROCESSING')
                                                <form action="{{ route('admin.orders.update', $Order->id) }}"
                                                      id="assign-{{ $Order->id }}" method="POST">
                                                    {{ csrf_field() }}
                                                    {{ method_field("PATCH") }}
                                ...
                                <div class="card-block">
                                    ...
                                    <div class="col-sm-9">
                                        ...
                                        <div class="card-btm row m-0">
                                            ...
                                            <a href="{{ url('admin/chat?order_id='.$Order->id) }}" target="_blank"
            ...
            render: function () {
                var listItems = this.state.data.map(function (item) {
                    ...
                    var cancel_div = <div className="delete-block">
                        <form action={`{{url('admin/orders/')}}/${item.id}`} id="assign-item.id"
                    ...
                    if (item.status == 'RECEIVED') {
                        ...
                        var assign1 = <a href={`{{url('admin/orders/')}}/${item.id}?t=pending&p=auto`}
                                         className="btn btn-danger btn-darken-3">{{trans("order.dispatcher.assign")}}</a>;
                                @else
                        var assign = <a href={`{{url('admin/orders/')}}/${item.id}?t=pending`}
                                        className="btn btn-danger btn-darken-3">{{trans("order.dispatcher.assign")}}</a>;
                        @endif
