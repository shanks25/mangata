    ...
    <div class="card">
        <ul class="nav nav-tabs row m-0 common-tab">
            <div class="col-sm-3 p-0">
                <li class="nav-item">
                    <a href="<?php echo e(url('shop/orders')); ?>?t=pending"
                       class="nav-link <?php if(Request::get('t')=='pending'): ?> active  <?php endif; ?>">Incoming Orders</a>
                </li>
            </div>
            <div class="col-sm-3 p-0">
                <li class="nav-item">
                    <a href="<?php echo e(url('shop/orders')); ?>?t=pickup"
                       class="nav-link <?php if(Request::get('t')=='pickup'): ?> active  <?php endif; ?>">Pickup Orders</a>
                </li>
            </div>
            <div class="col-sm-3 p-0">
                <li class="nav-item">
                    <a href="<?php echo e(url('shop/orders')); ?>?t=accepted"
                       class="nav-link <?php if(Request::get('t')=='accepted'): ?> active  <?php endif; ?>">Accepted Orders</a>
                </li>
            </div>
            <div class="col-sm-3 p-0">
                <li class="nav-item">
                    <a href="<?php echo e(url('shop/orders')); ?>?t=ongoing"
                       class="nav-link <?php if(Request::get('t')=='ongoing'): ?> active  <?php endif; ?>">Ongoing Orders</a>
                </li>
            </div>
            <div class="col-sm-3 p-0">
                <li class="nav-item">
                    <a href="<?php echo e(url('shop/orders')); ?>?t=cancelled"
                       class="nav-link <?php if(Request::get('t')=='cancelled'): ?> active  <?php endif; ?>">Cancelled Orders</a>
                </li>
            </div>
        </ul>
        ...
        <?php if(Request::get('t')=='pending'): ?>
            <div class="row">
                <form action="<?php echo e(url('shop/orders?t=pending')); ?>" method="GET" class="popup-form">
                    <div class="col-md-6">
            ...
            <div class="col-md-7">
                <div class="dis-left" id="container">
                    ...
                    <?php $__empty_1 = true; $__currentLoopData = $Orders; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $Order): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        ...
                        //dd($Orders);?>
                        <div class="card card-inverse pending-block row m-0 <?php if($Order->status == 'RECEIVED'): ?> bg-success <?php elseif($order_now ==1): ?> bg-primary   <?php else: ?> bg-danger <?php endif; ?>">
                                ...
                                <div class="col-sm-9">
                                    ...
                                    <div class="card-btm row m-0">
                                        <div class="form-group col-sm-4">
                                            <button class="btn <?php if($Order->status == 'RECEIVED'): ?> btn-success  <?php elseif($order_now ==1): ?> btn-primary <?php else: ?> btn-danger <?php endif; ?> btn-darken-3 orderlist"
                                                    data-id="<?php echo e($Order->id); ?>">Order List
                                            </button>
                                        </div>
                                        <?php if($Order->status=='ORDERED' && $Order->dispute!='CREATED'): ?>
                                            <div class="form-group col-sm-2">
                                                <form action="<?php echo e(route('shop.orders.update', $Order->id)); ?>"
                                                      id="assign-<?php echo e($Order->id); ?>" class="form-horizontal"
                                                      method="POST">
                                                    ...
                                                    <button type="button"
                                                            class="btn <?php if($Order->status == 'RECEIVED'): ?> btn-success  <?php else: ?> btn-danger <?php endif; ?> btn-darken-3">
                                            ...
                                            <div class="form-group col-sm-2">
                                                <form action="<?php echo e(route('shop.orders.update', $Order->id)); ?>"
                                                    ...
                                                    <button class="btn <?php if($Order->status == 'CANCELLED'): ?> btn-success  <?php else: ?> btn-danger <?php endif; ?> btn-darken-3">
                                                        <i class="ft-x"></i>
                                                    </button>
                                                </form>
                                            <!--  <form action="<?php echo e(route('shop.dispute.store')); ?>" id="assign-<?php echo e($Order->id); ?>" class="form-horizontal" method="POST">
                                                <?php echo e(csrf_field()); ?>
                                        ...
                                        <div class="form-group col-sm-2">
                                            ...
                                            <?php if($Order->status=='ASSIGNED'): ?>
                                                <span class="tag batch btn "><?php echo app('translator')->getFromJson('order.dispatcher.assigned'); ?></span>
                                            <?php endif; ?>
                                            <?php if($Order->status=='REACHED' || $Order->status=='PICKEDUP' || $Order->status=='ARRIVED' ): ?>
                                                <span class="tag batch btn"><?php echo e($Order->status); ?></span>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                </div>
    ...
    <div class="modal fade text-xs-left" id="order-ready-time">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action={`<?php echo e(url('shop/orders/')); ?>/${item.id}`} id={`cancel-${item.id}`} class="form-horizontal"
                          method="POST">
...
<?php $__env->startSection('scripts'); ?>
    ...
    <script type="text/javascript">
        ...
        $(document).on("click", ".received", function () {
            var id = $(this).data('id');
            var act_url = "<?php echo e(url('shop/orders/')); ?>/" + id;
        ...
        $(document).on("click", ".orderlist", function () {
            ...
            var order_date;
            $.ajax({
                url: "<?php echo e(url('shop/orders')); ?>/" + order_id,
                success: function (result) {
                    ...
                    $('.tot_amt').html("<span>: </span> <?php echo e(Setting::get('currency')); ?>" + result.Order.invoice.net);
    ...
    <script type="text/jsx">
        ...
        var MainComponent = React.createClass({
            ...
            componentDidMount: function () {
                $.ajax({
                    url: "<?php echo e(url('shop/orders?t='.Request::get('t'))); ?>",
                    type: "GET",
                    data: {'delivery_date': "<?php echo e(Request::get('delivery_date')); ?>"}
                })
            ...
            render: function () {
                var listItems = this.state.data.map(function (item) {
                    ...
                    if (item.status == 'ORDERED') {
                        ...
                        var assign = <form action={`<?php echo e(url('shop/orders/')); ?>/${item.id}`} id={`assign-${item.id}`}
                        ...
                        var cancel = <form action={`<?php echo e(url('shop/orders/')); ?>/${item.id}`} id={`assign-${item.id}`}
                                           className="form-horizontal" method="POST">
                            <input type="hidden" name="_token" value="<?php echo e(csrf_token()); ?>"/>
                            <input type="hidden" name="_method" value="PATCH"/>
                            <input type="hidden" name="status" value="CANCELLED"/>
